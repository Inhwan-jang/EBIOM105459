...
cyto<-lapply(data_total[c('pNK.05.1','pNK.2.1')], as.numeric)
...
x_ten_threefold= matrix(rep(t(x_ten),2),ncol=5,byrow=TRUE)
y=c(cyto$pNK.05.1[c(15:35)],cyto$pNK.2.1[c(15:35)])
...
beta = 1*c(2,2,2,2,2,2) ; gamma = c(1,0,0,0,0,0) ; c_vec = c(1.5)
K = 25 ;
sigma = 15
...
par(mfrow=c(2,3))
# plot(draw_beta[seq(from=5000000,to=10000000,by=1000),1], type="l")
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),1],freq=F)
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),2]*draw_gamma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=1),2],freq=F)
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),3]*draw_gamma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=1),3],freq=F)
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),4]*draw_gamma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=1),4],freq=F)
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),5]*draw_gamma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=1),5],freq=F)
hist(draw_beta[seq(from=round(n_iter*0.8)+1,to=n_iter,by=1),6]*draw_gamma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=1),6],freq=F)
...
par(mfrow=c(2,2))
plot(draw_c_vec[seq(from=round(n_iter*0.2)+1,to=n_iter,by=100),1],type="l")
plot(draw_sigma[seq(from=round(n_iter*0.2)+1,to=n_iter,by=100)],type="l")
plot(draw_K[seq(from=round(n_iter*0.2)+1,to=n_iter,by=100)],type="l")



library(R.matlab)
writeMat("draw_beta_nonlinear_solid_230622.mat",draw_c_vec=draw_c_vec,draw_beta=draw_beta, draw_gamma=draw_gamma,draw_sigma=draw_sigma, draw_K=draw_K)
writeMat("draw_beta_accept_nonlinear_solid_230622.mat",draw_c_vec_accept=draw_c_vec_accept,draw_beta_accept=draw_beta_accept,draw_gamma_accept=draw_gamma_accept, draw_sigma_accept=draw_sigma_accept,draw_K_accept=draw_K_accept)
#
